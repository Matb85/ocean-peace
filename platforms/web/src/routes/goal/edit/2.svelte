...
<script lang="ts">
  ...
  const hours: columnI = {
    units: "h",
    data: [...Array(6).keys()],
    current: parseInt(SM.goal.timeHours),
    multiplier: 1,
  };
  const minutes = timeInputConfig.minutesConfig(parseInt(SM.goal.timeMinutes) / 5);
  let h: number;
  let m: number;
  h = hours.current;
  m = minutes.current;
  ...
  Object.defineProperty(minutes, "current", {
    set: val => {
      m = val;
      SM.goal.timeMinutes = (m * minutes.multiplier).toString();
    },
    get: () => m,
  });

  const days = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"];
  let activeDays = JSON.parse(SM.goal.activeDays);
  /** filter days when saving to maintain order*/
  $: SM.goal.activeDays = JSON.stringify(days.filter(x => activeDays.includes(x)));

  const limitTypes = ["Notification", "Close app"];
  let type = SM.goal.limitActionType;
